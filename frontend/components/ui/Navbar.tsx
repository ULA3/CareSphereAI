    ...
    <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-slate-200 z-50 flex items-center px-4 gap-4 shadow-sm">
      ...
      <Link href="/" className="flex items-center gap-2.5 shrink-0 min-w-[220px]">
        ...
        <div className="leading-tight">
          <p className="font-bold text-slate-900 text-sm leading-none">CareSphere AI</p>
      ...
      <div ref={searchRef} className="flex-1 max-w-xl relative">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 text-sm rounded-lg border border-slate-200 bg-slate-50 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 focus:bg-white transition-all"
          />
          {query && (
            <button
        ...
        {searchOpen && (results.length > 0 || query.length > 0) && (
          <div className="absolute top-full mt-1.5 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-card-lg overflow-hidden z-50">
            {results.length === 0 && query.length > 0 ? (
              <div className="px-4 py-6 text-center text-sm text-slate-400">
                No patients found for &quot;<span className="font-medium text-slate-600">{query}</span>&quot;
              </div>
            ) : (
              <>
                <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
                  ...
                  <p className="text-xs text-slate-400">Click to view trends</p>
                </div>
                    ...
                    <li key={pt.id}>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-slate-900 truncate">{pt.name}</p>
      ...
      <div className="flex items-center gap-2 ml-auto shrink-0">
        ...
        <div className="flex rounded-lg overflow-hidden border border-slate-200 text-xs font-semibold">
          <button
            onClick={() => setLang('en')}
            className={`px-3 py-1.5 transition-colors ${lang === 'en' ? 'bg-brand-500 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
          >
            🇬🇧 EN
          </button>
          <button
            onClick={() => setLang('bm')}
            className={`px-3 py-1.5 transition-colors ${lang === 'bm' ? 'bg-brand-500 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
          >
            🇲🇾 BM
          </button>
        </div>
        ...
        <div ref={userRef} className="relative">
          <button
            ...
            className="flex items-center gap-2 h-9 px-2.5 rounded-lg hover:bg-slate-100 transition-colors"
          >
          ...
          {userOpen && (
            <div className="absolute right-0 top-full mt-1.5 w-52 bg-white border border-slate-200 rounded-xl shadow-card-lg overflow-hidden z-50 animate-slide-up">
              <div className="p-3 border-b border-slate-100">
                <p className="text-sm font-semibold text-slate-900">CareSphere Admin</p>
                <p className="text-xs text-slate-400 mt-0.5">admin@caresphere.my</p>
              </div>
              <div className="p-1.5">
                <Link href="/onboard" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-slate-700 hover:bg-slate-50 transition-colors">
